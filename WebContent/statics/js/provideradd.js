...
$(function(){
	...
	if(errorinfos.val() == null || errorinfos.val() == ""){
		a_workPicPaths.next().html("* 上传大小不能超过500K * 上传文件类型必须为：jpg、jpeg、png、pneg");
	}else{
		a_workPicPaths.next().html(errorinfos.val());
	}
	...
	proCode.on("blur",function(){
		if(proCode.val() != null && proCode.val() != ""){
			...
			validateTip(proCode.next(),{"color":"red"},imgNo+" 收藏编码不能为空，请重新输入",false);
		}
	}).on("focus",function(){
		//显示友情提示
